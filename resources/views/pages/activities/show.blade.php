...
<div class="w-full pt-44 pb-24 px-6 md:px-12 lg:px-24">
    ...
    <div class="max-w-4xl mx-auto mb-12">
        <a href="{{ route('activities.index') }}" class="group inline-flex items-center gap-2 text-[#00A651] font-bold text-xs md:text-sm uppercase tracking-widest hover:-translate-x-1 transition-transform duration-300">
    ...
    <article class="max-w-4xl mx-auto">
        ...
        <header class="mb-16">
            ...
            @if($activity->gallery && count($activity->gallery) > 0)
                <div class="rounded-[2.5rem] overflow-hidden shadow-2xl mb-16 aspect-[16/9]">
                    <img src="{{ Storage::url($activity->gallery[0]) }}" alt="{{ $activity->title }}" class="w-full h-full object-cover">
                </div>
            @endif
        </header>
        ...
        @if($activity->gallery && count($activity->gallery) > 1)
